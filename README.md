...
The installer copies the host plugin into the Web profile, deploys the Windows
bridge under `C:\MCP\dsh-vision-toolkit-windows-edge`, creates a shared 256-bit token, and
registers a current-user `DshVisionToolkitWindowsEdge` login task. It creates and owns `C:\MCP\dsh-vision-toolkit-windows-edge\.venv`, installs only the pinned Playwright client there, and uses installed Edge through `channel="msedge"`; it does not download another browser. The scheduled task and immediate launcher both use this venv’s windowless `pythonw.exe`.

Restart `dsh web` after installation. Windows and WSL must use mirrored networking,
